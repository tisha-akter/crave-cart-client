...
const Stats = () => {
  ...
  const getStatLabel = (stat) => {
    if (stat.labelKey) {
      return stat.labelPrefix ? `${language === 'en' ? 'Average' : 'গড়'} ${t[stat.labelKey]}` : t[stat.labelKey];
    }
  ...
  return (
    <section className="py-20 lg:py-32 bg-gradient-to-br from-gray-900 via-blue-900 to-purple-900 text-white relative overflow-hidden">
      ...
      <div className="absolute inset-0">
        ...
        <div className="absolute bottom-20 right-20 w-80 h-80 bg-gradient-to-r from-blue-500/10 to-purple-500/10 rounded-full blur-3xl animate-pulse" style={{animationDelay: '2s'}}></div>
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            ...
            return (
              ...
              >
                ...
                <div className="relative mb-6">
                  <div className={`w-20 h-20 mx-auto rounded-2xl bg-gradient-to-r ${stat.color} p-5 group-hover:scale-110 group-hover:rotate-6 transition-all duration-500 shadow-xl`}>
                    <IconComponent className="w-full h-full text-white" />
                  </div>
                  <div className={`absolute inset-0 w-20 h-20 mx-auto rounded-2xl bg-gradient-to-r ${stat.color} opacity-30 blur-xl group-hover:blur-2xl transition-all duration-500`}></div>
                </div>

                {/* Number */}
                <div className={`text-3xl lg:text-4xl font-bold mb-3 bg-gradient-to-r ${stat.color} bg-clip-text text-transparent group-hover:scale-110 transition-transform duration-300`}>
                  {stat.number}
                </div>
        ...
        <CTA
          ...
          primaryButton={{
            text: language === 'en' ? 'Join Our Community' : 'আমাদের কমিউনিটিতে যোগ দিন',
            variant: 'primary',
            size: 'large',
